...
このアプリケーションをインターネットに公開してデモとして見せる場合、第三者によるデータの書き込みを防ぐ「デモモード」を有効化できます。

### デモモードの機能

デモモード有効時：
- 画面上部に「デモモード」バナーが表示されます
- 新規ユーザー登録（サインアップ）が無効化されます
- すべての書き込み操作（注文、レビュー投稿、プロフィール更新など）がブロックされます
- 読み取り操作（商品一覧、注文履歴の閲覧など）は通常通り動作します
- ログインページでデモ用アカウントをワンクリックで入力できます
...
デモモードを無効にするには、環境変数を `false` に変更してアプリを再起動します：
